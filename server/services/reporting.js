...
export function parseReportHour(input, opts = {}) {
  ...
  let m = t.match(/^(\d{1,2})\s*:\s*(\d{2})\s*(ص|م|صباح|صباحا|مساء|مساءا|مسائ|ليلا)?$/);
  if (m) {
  ...
  m = t.match(/^(?:الساعة\s*)?(\d{1,2})\s*(?:الساعة|ساعة)?\s*(مساء|مساءا|مسائ|ليلا|ليل|الليل|عصرا|عصر|ظهرا|ظهر|الظهر|منتصف الليل|منتصف|الصباح|صباحا|صباح|صبح|فجرا|فجر|م|ص)?$/);
  if (!m) return null;
  let h = +m[1];
  const part = m[2] || '';
  if (/منتصف/.test(t)) h = 0;   // «12 منتصف الليل» = 00:00
  else if (/صباح|صبح|فجر/.test(part)) { if (h === 12) h = 0; }
